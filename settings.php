	...
	**/
		...
		**/
		function enqueue_admin_custom_scripts_and_styles() {
			...
			wp_enqueue_script( 'wp_sap_admin_chart', plugins_url( '/templates/assets/js/Chart.min.js', __FILE__ ), array('wp_sap_admin'),'100018', false );
			wp_enqueue_script( "wp_sap_timepicker_script", plugins_url('/templates/assets/js/jquery.timepicker.js', __FILE__ ), array( 'jquery','jquery-ui-core' ) );
		...
		**/		
		public function add_menu() {
			...
			add_menu_page( 'WP Survey & Poll', 'WP Survey & Poll', 'manage_options', 'wp_sap', array( &$this, 'plugin_settings_page' ), 'dashicons-id-alt', '65.013' );
		}
		/**
		* Menu Callback
		**/		
		...
		public function ajax_wpspsurvey() {
			...
			if ( isset( $_SERVER[ 'HTTP_X_REQUESTED_WITH' ] ) && ( $_SERVER[ 'HTTP_X_REQUESTED_WITH' ] == 'XMLHttpRequest' ) ) {
				...
				if ( isset( $_REQUEST[ 'options' ] ) ) {
					$survey_options = sanitize_text_field( $_REQUEST[ 'options' ] );
				}
				else {
					$survey_options = "";
				}
				if ( isset( $_REQUEST[ 'qa' ] ) ) {
					$survey_qa = sanitize_text_field( $_REQUEST[ 'qa' ] );
				}
				else {
					$survey_qa = "";
				}
				...
				if ( $_REQUEST[ 'sspcmd' ] == "save" ) {
					$wpsap_ltime = get_option( 'wpsap_ltime' );
					if ( empty( $wpsap_ltime ) ) {
						$date = date_create();
						update_option( 'wpsap_ltime', serialize( date_timestamp_get( $date ) ) );
					}
				if ( $survey_check > 0 ) {
				//update survey
