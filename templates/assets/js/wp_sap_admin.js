jQuery( window ).load( function() {
  ...
  (function() {        
    ...
    var refresh = function () {
		if ( played_question > 0 ) 	{	
			if ( surveysystem( '#' + active_survey + ' .display_style').val() == "bottom" ) {
				surveysystem( '#survey' ).animate({ bottom: '10px' }, parseInt( surveysystem( '#' + active_survey + ' .wp_sap_animation_speed_value').val().replace( "Animation Speed: ", "" ).replace( "sec", "" ) * 1000 ), 'easeOutBounce', function(){} )
			}
			if ( surveysystem( '#' + active_survey + ' .display_style').val() == "center" ) {
				surveysystem( '#survey' ).animate({ top: ( ( surveysystem( window ).height() - surveysystem( '#survey' ).height() ) / 2 ) + "px" }, parseInt( surveysystem( '#' + active_survey + ' .wp_sap_animation_speed_value' ).val().replace( "Animation Speed: ", "" ).replace( "sec", "" ) * 1000 ), 'easeOutBounce', function() {} )
			}
			if ( surveysystem( '#' + active_survey + ' .display_style').val() == "top" ) {
				surveysystem( '#survey' ).animate({ top: '10px' }, parseInt( surveysystem( '#' + active_survey + ' .wp_sap_animation_speed_value').val().replace( "Animation Speed: ", "" ).replace( "sec", "" ) * 1000 ), 'easeOutBounce', function() {} )
			}
		}
    };
})();
	...
	function arrayHasOwnIndex(array, prop) {
 	...
 	surveysystem( "body" ).on( "click", ".sshortcode",function() {
		var survey_id = surveysystem( this ).parent().parent().attr( "id" );
		oldhtml = surveysystem( "div.help-dialogs" ).html();
		var newhtml = oldhtml.replace( /[0-9]+/g, survey_id );
		surveysystem( "div.help-dialogs" ).html( newhtml );
	});

 	surveysystem( "body" ).on( "click", ".srshortcode",function() {
		var survey_id = surveysystem( this ).parent().parent().attr( "id" );
		oldhtml = surveysystem( "div.help-dialogs" ).html();
		var newhtml = oldhtml.replace( /[0-9]+/g, survey_id );
		surveysystem( "div.help-dialogs" ).html( newhtml );
...
function initialize_question_accordions(survey_id)
{
	surveysystem( "#" + survey_id + " #new_questions" ).accordion({
	  ...
	  activate: function( event, ui ) {
        ...
        stop: function( event, ui ) {
		  ...
		  organize_question_subheaders( survey_id );
        }
      });
};
	
	function organize_question_subheaders( survey_id ) {
		  surveysystem( ".question_text" ).each( function( index ) {
			surveysystem( this ).parent().parent().parent().parent().parent().children( "h3" ).append( "<span class='question-subheader'>" + surveysystem( this ).val().replace(/(<([^>]+)>)/ig,"") + '</span>' );
		  })
	}
 	...
 	surveysystem("body").on( "click", ".save_survey",function() {
		...
		var thiscount = surveysystem("#"+survey_id+" #"+thisquestion+" p span#answer_count"+(index2+1)).text().split(" - ");
			...
			var data = {
				...
				checker = setTimeout(function(){if (surveysystem(buttonspan).html()!='<input type="submit" name="save_survey" class="save_survey button" value="SAVE">'||surveysystem(buttonspan).html()!='<input type="submit" name="save_survey" class="save_survey button" value="SAVE">')
			{
   ...
   surveysystem( "#dialog-confirm3" ).dialog({
      ...
      buttons: {
		...
		window.open(
		  'https://modalsurvey.pantherius.com/pricing',
		  '_blank'
		);
        }
      }
